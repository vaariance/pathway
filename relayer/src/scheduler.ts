...
export const handler: Handler = async () => {
  ...
  try {
    const [{ Items: waiting_items }, { Items: pending_items }, { Items: failed_items }] =
      await Promise.all([
    ...
    const now = Date.now()
    const pending_ready = (pending_items as ReceiveMessageFormat[] | undefined)?.filter(
      (item) => new Date(item.submitted_at).getTime() + item.block_confirmation_in_ms < now
    )
    let failed_ready = (failed_items as ReceiveMessageFormat[] | undefined)?.filter(
      (item) => item.retry_at && new Date() > new Date(item.retry_at)
    )
