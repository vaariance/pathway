...
app.get('/message/:partition_key', async function (req, res) {
  ...
  try {
    ...
    if (Item) {
      ...
      res.status(404).json({ error: 'Could not find tx with provided "hash or key"' })
    }
  } catch (error) {
    console.error(error)
...
app.post('/message/new', async function (req, res) {
  ...
  const { tx_hash, partition_key }: { tx_hash?: string; partition_key?: string } = req.query
  ...
  try {
    const { Item } = await dynamodb_client.send(new GetCommand(params as GetCommandInput))
    if (Item) {
      res.status(409).json({ error: 'Tx with provided "hash or key" already exists' })
    }
  } catch (error) {
    console.error(error)
    res.status(500).json({ error: 'Could not retreive tx' })
